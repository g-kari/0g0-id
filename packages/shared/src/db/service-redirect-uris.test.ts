...
describe("listRedirectUris", () => {
  ...
  it("service_idでフィルタリングするSQLを使用する", async () => {
    ...
    expect(db.prepare).toHaveBeenCalledWith(expect.stringContaining("WHERE service_id = ?"));
  });

  it("ORDER BY created_at ASCでソートする", async () => {
    const db = makeD1Mock(null, []);
    await listRedirectUris(db, "service-1");
    expect(db.prepare).toHaveBeenCalledWith(expect.stringContaining("ORDER BY created_at ASC"));
  });
});
