...
export async function addRedirectUri(
  ...
  await db
    .prepare('INSERT INTO service_redirect_uris (id, service_id, uri) VALUES (?, ?, ?)')
    .bind(params.id, params.serviceId, params.uri)
    .run();

  const uri = await db
    .prepare('SELECT * FROM service_redirect_uris WHERE id = ?')
    .bind(params.id)
...
export async function deleteRedirectUri(db: D1Database, id: string): Promise<void> {
  await db.prepare('DELETE FROM service_redirect_uris WHERE id = ?').bind(id).run();
}
