...
describe("listServicesByOwner", () => {
  ...
  it("owner_user_id = ? のSQLを呼ぶ", async () => {
    ...
    expect(db.prepare).toHaveBeenCalledWith(expect.stringContaining("WHERE owner_user_id = ?"));
...
describe("rotateClientSecret", () => {
  it("新しいclient_secret_hashでUPDATEする", async () => {
    ...
    expect(result).toEqual(updated);
    expect(db.prepare).toHaveBeenCalledWith(expect.stringContaining("client_secret_hash = ?"));
...
describe("transferServiceOwnership", () => {
  it("新しいowner_user_idでUPDATEする", async () => {
    ...
    expect(result).toEqual(updated);
    expect(db.prepare).toHaveBeenCalledWith(expect.stringContaining("owner_user_id = ?"));
