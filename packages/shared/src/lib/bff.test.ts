...
describe('exchangeCodeAtIdp', () => {
  it('認可コード交換に成功した場合はok:trueとExchangeResultを返す', async () => {
    ...
    const idpFetch = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ data: exchangeResult }), { status: 200 })
    );
    const env = {
      IDP: { fetch: idpFetch },
      IDP_ORIGIN: 'https://id.0g0.xyz',
  ...
  it('INTERNAL_SERVICE_SECRETが設定されている場合はX-Internal-Secretヘッダーを付与する', async () => {
    const idpFetch = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ data: { access_token: 'at', refresh_token: 'rt', user: { id: 'u1', email: 'e@e.com', name: 'N', role: 'user' } } }), { status: 200 })
    );
    const env = {
      IDP: { fetch: idpFetch },
      IDP_ORIGIN: 'https://id.0g0.xyz',
