<script lang="ts">
	...
	const errorMsg = errorCode ? (errorMessages[errorCode] ?? '認証エラーが発生しました') : null;
</script>
...
<div class="min-h-screen flex items-center justify-center p-4">
	<div class="w-full max-w-sm">
		<div class="card">
			<h1 class="text-2xl font-bold mb-1" style="color: var(--color-text)">0g0 ID</h1>
			<p class="text-sm mb-6" style="color: var(--color-muted)">アカウントにサインインしてください</p>

			{#if errorMsg}
				<div class="alert alert-error mb-4" role="alert">{errorMsg}</div>
			{/if}

			<div class="flex flex-col gap-3">
				<a href="/auth/login?provider=google" class="btn btn-google">
				...
				<a href="/auth/login?provider=line" class="btn btn-line">
				...
				<a href="/auth/login?provider=twitch" class="btn btn-twitch">
				...
				<a href="/auth/login?provider=github" class="btn btn-github">
					<svg width="18" height="18" viewBox="0 0 24 24" fill="white" aria-hidden="true">
				...
				<a href="/auth/login?provider=x" class="btn btn-x">
					<svg width="18" height="18" viewBox="0 0 24 24" fill="white" aria-hidden="true">
