<script lang="ts">
  ...
  let unlinking = $state<OAuthProvider | null>(null);

  const linkedCount = $derived(statuses.filter((s) => s.connected).length);

  onMount(async () => {
    const res = await apiFetch<{ data: ProviderStatus[] }>('/api/providers');
    if ('error' in res) {
  ...
  async function unlink(provider: OAuthProvider): Promise<void> {
    if (!confirm(`${PROVIDER_LABELS[provider]} との連携を解除しますか？`)) return;
    unlinking = provider;
    const res = await apiFetch(`/api/providers/${provider}`, { method: 'DELETE' });
    unlinking = null;
    if (res && 'error' in res) {
      showToast(res.error.message, 'error');
    } else {
      statuses = statuses.map((s) => (s.provider === provider ? { ...s, connected: false } : s));
      showToast(`${PROVIDER_LABELS[provider]} の連携を解除しました`, 'success');
    }
  }
</script>

<svelte:head>
  <title>SNS連携 — 0g0 ID</title>
</svelte:head>
...
<div class="min-h-screen p-4" style="background:var(--color-surface);">
  <div style="max-width:480px; margin:0 auto; display:flex; flex-direction:column; gap:1.5rem; padding:2rem 0;">
    ...
    <div>
      <h1 style="font-size:1.5rem; font-weight:700;">SNS連携</h1>
      <p style="color:var(--color-muted); font-size:0.875rem; margin-top:4px;">
        複数のSNSアカウントを同一IDに連携できます
      </p>
    </div>
    ...
    {:else}
      <div class="card" style="padding:0; overflow:hidden; display:flex; flex-direction:column; gap:0;">
        {#each statuses as status, i}
          <div style="display:flex; align-items:center; justify-content:space-between; padding:1rem 1.5rem; {i > 0 ? 'border-top:1px solid var(--color-border);' : ''}">
            <div style="display:flex; align-items:center; gap:0.75rem;">
              ...
              {:else if status.provider === 'twitch'}
              ...
              {:else if status.provider === 'github'}
                <svg width="20" height="20" viewBox="0 0 24 24" fill="var(--color-text)" aria-hidden="true">
              ...
              {:else if status.provider === 'x'}
                <svg width="20" height="20" viewBox="0 0 24 24" fill="var(--color-text)" aria-hidden="true">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                </svg>
              {/if}
              <div>
                <div style="font-weight:500; font-size:0.9375rem;">{PROVIDER_LABELS[status.provider]}</div>
                <div style="font-size:0.75rem; color:var(--color-muted); margin-top:2px;">
                  {status.connected ? '連携済み' : '未連携'}
                </div>
              </div>
            </div>
            ...
            {#if status.connected}
              <button
                ...
                disabled={linkedCount <= 1 || unlinking === status.provider}
                title={linkedCount <= 1 ? '最後の連携プロバイダーは解除できません' : undefined}
                onclick={() => unlink(status.provider)}
              >
                {unlinking === status.provider ? '解除中...' : '連携解除'}
              </button>
            {:else}
              <form method="post" action="/auth/link">
                <input type="hidden" name="provider" value={status.provider} />
                <button type="submit" class="btn btn-primary btn-sm">連携する</button>
              </form>
            {/if}
          </div>
        {/each}
      ...
      {#if linkedCount <= 1}
        <p class="alert alert-error" style="font-size:0.8125rem;">
          連携中のプロバイダーが1つしかないため、解除できません。先に別のプロバイダーを連携してください。
        </p>
      {/if}
    {/if}
  </div>
