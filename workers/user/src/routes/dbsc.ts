...
const dbscLogger = createLogger("user-dbsc");
 ...
 *
 * CSRF 対策:
 * - __Host-user-session Cookie を必須とすることで、ログイン済みユーザーのブラウザ以外を排除。
...
app.post("/refresh", async (c) => {
  ...
  if (!proofHeader) {
    ...
    if (!challengeResp.ok) {
      ...
      return c.json({ error: { code: "INVALID_REQUEST", message: "Cannot issue challenge" } }, 400);
    }
  ...
  if (!verifyResp.ok) {
    ...
    if (verifyResp.status >= 500) {
      return c.json({ error: { code: "INTERNAL_ERROR", message: "Failed to verify proof" } }, 500);
    }
