...
function buildApp(idpFetch: (req: Request) => Promise<Response>) {
  const app = new Hono<{
    Bindings: { IDP: { fetch: typeof idpFetch }; IDP_ORIGIN: string; SESSION_SECRET: string; INTERNAL_SERVICE_SECRET?: string };
  }>();
...
describe('user BFF — /api/device', () => {
  describe('POST /verify — ユーザーコード検証', () => {
    ...
    it('IdPが404を返した場合はそのステータスコードをそのまま返す', async () => {
      const idpFetch = vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ error: { code: 'INVALID_CODE', message: 'Unknown user code' } }), {
          status: 404,
          headers: { 'Content-Type': 'application/json' },
        })
      );
  ...
  describe('POST /approve — デバイス承認/拒否', () => {
    ...
    it('IdPが400を返した場合はそのステータスコードを返す', async () => {
      const idpFetch = vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ error: { code: 'CODE_EXPIRED', message: 'Device code expired' } }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' },
        })
      );
