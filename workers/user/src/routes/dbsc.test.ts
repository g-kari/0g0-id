...
describe("user /auth/dbsc/start", () => {
  ...
  it("正規 JWT → IdP に bind を委譲して 200 を返す", async () => {
    ...
    const body = (await res.json()) as { refresh_url: string; credentials: { name: string }[] };
    expect(body.refresh_url).toBe("/auth/dbsc/refresh");
    // user BFF のクッキー名が返ること（admin と取り違えていないこと）
    expect(body.credentials[0]?.name).toBe("__Host-user-session");
...
describe("user /auth/dbsc/refresh", () => {
  beforeEach(() => vi.resetAllMocks());
