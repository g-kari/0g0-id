...
import { userAuthRateLimitMiddleware, userApiRateLimitMiddleware } from "./rate-limit";
...
const baseUrl = "https://user.example.com";
...
type MockEnv = {
  ...
  RATE_LIMITER_USER_AUTH?: MockRateLimiter;
  RATE_LIMITER_USER_API?: MockRateLimiter;
};

function buildApp(middleware: typeof userAuthRateLimitMiddleware, envOverrides?: Partial<MockEnv>) {
  const app = new Hono();
  ...
  const env: MockEnv = {
    ...
    SELF_ORIGIN: "https://user.example.com",
...
describe("userAuthRateLimitMiddleware", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });

  it("開発環境でバインディング未設定の場合はスキップして200を返す", async () => {
    const { app, env } = buildApp(userAuthRateLimitMiddleware, {
      RATE_LIMITER_USER_AUTH: undefined,
      SELF_ORIGIN: "http://localhost:5173",
    });
  ...
  it("レートリミット成功の場合は200を返す", async () => {
    ...
    const { app, env } = buildApp(userAuthRateLimitMiddleware, {
      RATE_LIMITER_USER_AUTH: { limit: mockLimit },
    });
    const res = await app.request(
      new Request(`${baseUrl}/test`, {
...
describe("userApiRateLimitMiddleware", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("レートリミット成功の場合は200を返す", async () => {
    const mockLimit = vi.fn().mockResolvedValue({ success: true });
    const { app, env } = buildApp(userApiRateLimitMiddleware, {
      RATE_LIMITER_USER_API: { limit: mockLimit },
    });
    const res = await app.request(
      new Request(`${baseUrl}/test`, {
