/**
 * admin BFF デプロイ前のプリフライトチェック（issue #155 DBSC Phase 3）
 *
 * `DBSC_ENFORCE_SENSITIVE` が wrangler secret として登録されているかだけを
 * 確認し、未登録なら警告する。値そのものは wrangler secret list では見えない
 * ため値検査はしない（runtime 側の `isEnforceEnabled` で "true" 正規化済み）。
 *
 * 終了コード:
...
import { runPreflight, type PreflightRunner } from "../../../packages/shared/src/lib/bff-preflight";

const WORKER_NAME = "admin";
...
const outcome = runPreflight(WORKER_NAME, runner);
// `runPreflight` は純粋関数として設計されており process.exit しないので、
