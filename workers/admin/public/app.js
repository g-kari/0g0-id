...
(function () {
          ...
          const rows = data.data.map(function (s) {
            return '<tr>' +
              ...
              '<td style="white-space:nowrap;">' +
                '<button class="btn btn-secondary btn-sm mr-1" data-users-service-id="' + escHtml(s.id) + '" data-users-service-name="' + escHtml(s.name) + '">ユーザー</button>' +
    ...
    function openServiceUsers(serviceId, serviceName) {
      ...
      fetch('/api/services/' + serviceId + '/users', { credentials: 'same-origin' })
        ...
        .then(function (data) {
          ...
          if (data.error) {
            serviceUsersContent.innerHTML = '<p style="color:var(--error);font-size:0.875rem;">取得に失敗しました</p>';
            return;
          }
          var items = data.data || [];
          if (items.length === 0) {
            serviceUsersContent.innerHTML = '<p style="color:var(--text-muted);font-size:0.875rem;">このサービスを認可したユーザーがいません</p>';
            return;
          }
        ...
        .catch(function () {
          hideProgress();
          if (serviceUsersContent) serviceUsersContent.innerHTML = '<p style="color:var(--error);font-size:0.875rem;">通信エラーが発生しました</p>';
        });
    }

    if (serviceUsersCloseBtn && serviceUsersModal) {
      serviceUsersCloseBtn.addEventListener('click', function () { serviceUsersModal.hidden = true; });
    }
  ...
  if (path === '/users') {
      ...
      if (tabName === 'login-history') {
        ...
        fetch('/api/users/' + userId + '/login-history', { credentials: 'same-origin' })
          ...
          .then(function (data) {
            ...
            if (data.error) { panel.innerHTML = '<p style="color:var(--error);font-size:0.875rem;">取得に失敗しました</p>'; return; }
            var items = data.data || [];
            if (items.length === 0) { panel.innerHTML = '<p style="color:var(--text-muted);font-size:0.875rem;">ログイン履歴がありません</p>'; return; }
            panel.innerHTML =
              ...
              items.map(function (item) {
                return '<tr>' +
                  ...
                  '<td class="mono">' + escHtml(item.ip_address || '—') + '</td>' +
          ...
          .then(function (data) {
            ...
            var tableHtml = items.length === 0
              ...
              : '<div class="table-wrap" style="margin-bottom:1rem;"><table aria-label="セッション一覧">' +
                ...
                items.map(function (item) {
                  return '<tr>' +
                    '<td>' + escHtml(item.service_name || 'IdPセッション') + '</td>' +
                    '<td>' + new Date(item.created_at).toLocaleString('ja-JP') + '</td>' +
                    '<td>' + new Date(item.expires_at).toLocaleString('ja-JP') + '</td>' +
                    '</tr>';
                }).join('') +
                '</tbody></table></div>';
    ...
    function openUserDetail(userId, userName) {
      ...
      // 全タブパネルを空にする
      Object.keys(tabPanels).forEach(function (key) {
        if (tabPanels[key]) { tabPanels[key].innerHTML = ''; tabPanels[key].hidden = true; }
      });
