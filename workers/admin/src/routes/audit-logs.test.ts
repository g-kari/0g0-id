...
describe("admin BFF — /api/audit-logs", () => {
  describe("GET / — 監査ログ一覧", () => {
    ...
    it("admin_user_idフィルターをIdPに転送する", async () => {
      ...
      const calledUrl = new URL(vi.mocked(idpFetch).mock.calls[0][0].url);
      expect(calledUrl.searchParams.get("admin_user_id")).toBe(
    ...
    it("複数フィルターを同時にIdPに転送する", async () => {
      ...
      await app.request(
        "/api/audit-logs?admin_user_id=00000000-0000-0000-0000-000000000001&target_id=00000000-0000-0000-0000-000000000002&action=user.ban",
        {
          headers: { Cookie: `${SESSION_COOKIE}=${await makeSessionCookie()}` },
        },
      );
