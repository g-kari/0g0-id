...
app.get('/suspicious-logins', async (c) => {
  ...
  if (hoursRaw !== undefined) {
    if (!/^\d+$/.test(hoursRaw)) {
      return c.json({ error: { code: 'INVALID_PARAMETER', message: 'hours must be a positive integer' } }, 400);
    }
    const hours = parseInt(hoursRaw, 10);
    if (hours < 1 || hours > 720) {
      return c.json({ error: { code: 'INVALID_PARAMETER', message: 'hours must be between 1 and 720' } }, 400);
    }
  ...
  if (minCountriesRaw !== undefined) {
    if (!/^\d+$/.test(minCountriesRaw)) {
      return c.json({ error: { code: 'INVALID_PARAMETER', message: 'min_countries must be a positive integer' } }, 400);
    }
    const minCountries = parseInt(minCountriesRaw, 10);
    if (minCountries < 1 || minCountries > 100) {
      return c.json({ error: { code: 'INVALID_PARAMETER', message: 'min_countries must be between 1 and 100' } }, 400);
    }
