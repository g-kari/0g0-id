...
describe("rotateServiceSecretTool", () => {
  ...
  it("サービスが見つからない場合はエラー", async () => {
    ...
    const result = await rotateServiceSecretTool.handler(
...
describe("updateServiceTool", () => {
  ...
  it("nameを更新する", async () => {
    ...
    const result = await updateServiceTool.handler(
      { service_id: "svc-1", name: "Updated Name" },
      mockContext,
    );
...
describe("listServiceUsersTool", () => {
  ...
  it("pageとlimitを指定できる", async () => {
    ...
    const result = await listServiceUsersTool.handler(
      { service_id: "svc-1", page: 3, limit: 10 },
      mockContext,
    );
