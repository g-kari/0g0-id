...
describe('McpServer', () => {
  ...
  describe('tools/call', () => {
    ...
    it('未登録ツール名 → Unknown tool エラー', async () => {
      const server = new McpServer();
      const res = await server.handleRequest(makeRequest('tools/call', { name: 'nonexistent' }), mockContext);
      expect(res.error?.code).toBe(-32602);
  ...
  describe('id の受け渡し', () => {
    it('リクエストのidがレスポンスにそのまま反映される', async () => {
      const server = new McpServer();
      const res = await server.handleRequest(makeRequest('ping', undefined, 'req-abc'), mockContext);
