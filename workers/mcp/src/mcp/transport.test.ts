...
describe('createMcpRoutes - POST /mcp', () => {
  ...
  it('initialize以外で無効なセッションIDの場合は-32600エラーを返す', async () => {
    ...
    const app = buildApp();
    const res = await app.request(
      makePostRequest({ jsonrpc: '2.0', id: 3, method: 'tools/list' }, {
  ...
  it('有効なセッションIDで通常のリクエストを処理できる', async () => {
    ...
    const app = buildApp();
    const res = await app.request(
      makePostRequest({ jsonrpc: '2.0', id: 4, method: 'tools/list' }, {
  ...
  it('idがレスポンスにそのまま反映される', async () => {
    ...
    const app = buildApp();
    const res = await app.request(
      makePostRequest({ jsonrpc: '2.0', id: 'req-abc-123', method: 'ping' }, {
        'Mcp-Session-Id': 'valid-session-id',
      }),
      undefined,
      mockEnv as unknown as Record<string, string>,
...
describe('createMcpRoutes - DELETE /mcp', () => {
  ...
  it('セッションIDがある場合はセッションを削除して204を返す', async () => {
    ...
    expect(vi.mocked(deleteMcpSession)).toHaveBeenCalledWith(
      mockEnv.DB,
      'session-to-delete',
    );
  });
