...
describe("GET /api/admin/audit-logs", () => {
  ...
  it("未認証は 401 を返す", async () => {
    ...
    const app = createApp();
    const res = await app.request(
      makeRequest("/api/admin/audit-logs", { withAuth: false }),
      undefined,
      mockEnv,
    );
...
describe("GET /api/admin/audit-logs/stats", () => {
  const mockStats = {
    ...
    admin_stats: [{ admin_user_id: "admin-user-id", count: 8 }],
    daily_stats: [
  ...
  it("days=7 を指定できる", async () => {
    ...
    const app = createApp();
    const res = await app.request(
      makeRequest("/api/admin/audit-logs/stats?days=7"),
      undefined,
      mockEnv,
    );
  ...
  it("days が範囲外（999）の場合は 400 を返す", async () => {
    ...
    const app = createApp();
    const res = await app.request(
      makeRequest("/api/admin/audit-logs/stats?days=999"),
      undefined,
      mockEnv,
    );
  ...
  it("days が範囲外（0）の場合は 400 を返す", async () => {
    ...
    const app = createApp();
    const res = await app.request(
      makeRequest("/api/admin/audit-logs/stats?days=0"),
      undefined,
      mockEnv,
    );
  ...
  it("不正な days（文字列）の場合は 400 を返す", async () => {
    ...
    const app = createApp();
    const res = await app.request(
      makeRequest("/api/admin/audit-logs/stats?days=abc"),
      undefined,
      mockEnv,
    );
  ...
  it("未認証は 401 を返す", async () => {
    ...
    const app = createApp();
    const res = await app.request(
      makeRequest("/api/admin/audit-logs/stats", { withAuth: false }),
      undefined,
      mockEnv,
    );
