...
import { issueTokenPair, buildTokenResponse, REFRESH_TOKEN_TTL_MS, ACCESS_TOKEN_TTL_SECONDS } from './token-pair';
...
describe('issueTokenPair', () => {
  beforeEach(() => {
    ...
    vi.spyOn(crypto, 'randomUUID').mockReturnValue('mock-uuid-1234' as ReturnType<typeof crypto.randomUUID>);
  });
  ...
  it('clientId が指定された場合は cid に含める', async () => {
    await issueTokenPair(mockDb, mockEnv, mockUser, { serviceId: 'service-1', clientId: 'client-abc' });
  ...
  it('scope が指定された場合は signAccessToken に渡す', async () => {
    await issueTokenPair(mockDb, mockEnv, mockUser, { serviceId: 'service-1', scope: 'openid profile' });
  ...
  it('familyId が指定された場合はそれを使う', async () => {
    await issueTokenPair(mockDb, mockEnv, mockUser, { serviceId: 'service-1', familyId: 'existing-family' });
  ...
  it('scope が指定された場合 createRefreshToken にそれを渡す', async () => {
    await issueTokenPair(mockDb, mockEnv, mockUser, { serviceId: 'service-1', scope: 'openid email' });
