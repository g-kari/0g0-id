...
id Worker は Cloudflare Workers の Rate Limiting バインディングを使用して、エンドポイントごとにリクエスト数を制限する。

### 6.1 仕組み

Cloudflare Workers の `[[unsafe.bindings]]` で `type = "ratelimit"` バインディングを定義し、ミドルウェアからバインディングの `limit()` メソッドを呼び出す。制限超過時は HTTP 429（Too Many Requests）を返す。本番環境（`IDP_ORIGIN` が `https://` で始まる場合）のみ適用され、ローカル開発時はスキップされる。

### 6.2 バインディング定義（wrangler.toml）
...
各バインディングは `namespace_id` で一意に識別される。`simple.limit` が期間内の最大リクエスト数、`simple.period` が秒単位のウィンドウ長。

### 6.3 レートリミッター一覧

| バインディング名             | namespace_id | 制限        | キー戦略                                                            | 対象エンドポイント                                                                                     |
| ---------------------------- | ------------ | ----------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `RATE_LIMITER_AUTH`          | 1001         | 20 req/60s  | クライアント IP                                                     | `/auth/authorize`, `/auth/login`, `/auth/callback`                                                     |
| `RATE_LIMITER_EXTERNAL`      | 1002         | 200 req/60s | Basic 認証の `client_id`、フォールバックで IP                       | `/api/external/*`, `/api/userinfo`, `/api/token/introspect`, `/api/token/revoke`                       |
| `RATE_LIMITER_TOKEN`         | 1003         | 30 req/60s  | クライアント IP                                                     | `/auth/exchange`, `/auth/refresh`, `/auth/logout`, `/auth/dbsc/*`, `/api/token`, `/api/device/code` 等 |
| `RATE_LIMITER_DEVICE_VERIFY` | 1004         | 10 req/60s  | 認証済みユーザーの `sub`、フォールバックで IP                       | `/api/device/verify`（RFC 8628 デバイスコード検証）                                                    |
| `RATE_LIMITER_TOKEN_CLIENT`  | 1005         | 100 req/60s | Basic 認証またはリクエストボディの `client_id`、フォールバックで IP | `/api/token`（トークン発行エンドポイント）                                                             |

### 6.4 制限値の変更

制限値を変更するには `workers/id/wrangler.toml` の該当バインディングの `simple` を編集し、再デプロイする。

```toml
# 例: 認証エンドポイントの制限を 20 → 30 に引き上げ
[[unsafe.bindings]]
name = "RATE_LIMITER_AUTH"
type = "ratelimit"
namespace_id = "1001"
...
> ⚠️ `namespace_id` は変更しないこと。変更すると既存のレートリミットカウンターがリセットされ、別のリミッターとして扱われる。
